...
class File extends Field
{
    ...
    {
        $this->script = <<<EOT
$("input{$this->getElementClassSelector()}").fileinput({$options}).on("filebatchselected", function(event, data) {
    if(data[0].name != $('#xiapi_waybill_no').val() + '.pdf') {
        alert('PDF名字需要與面單條碼一致')
        $(".close.fileinput-remove span").click()
        return false;
    }
});
EOT;
